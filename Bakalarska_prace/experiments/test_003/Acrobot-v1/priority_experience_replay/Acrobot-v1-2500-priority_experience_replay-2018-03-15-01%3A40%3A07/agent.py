...
class Agent:
    def __init__(self, state_size, action_size):
        ...
        self.epsilonDiminution = (self.startEpsilon - self.endEpsilon) / 150000#(self.startEpsilon - self.endEpsilon)/(episodes * 2)                  # Hodnota snizovani epsilonu
    ...
    def remember(self, state, action, reward, nextState, done, rand_agent):
        if rand_agent == True:
            ...
            if done == 1:
                ...
                aNet = self.netTarget.predict(np.array([nextState]))[0]
                target_f[0][action] = reward + self.gamma * np.max(aNet)

            obs_error = abs(obs_error - target_f[0][action])

        self.memory.add_observation((state, action, reward, nextState, done), obs_error)
