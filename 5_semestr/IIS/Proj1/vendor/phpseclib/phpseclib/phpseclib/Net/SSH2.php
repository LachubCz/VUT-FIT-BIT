 ...
 *    if (!$ssh->sessions('username', 'password')) {
 *        exit('Login Failed');
 *    }
 *
 *    echo $ssh->exec('pwd');
    ...
    */
    ...
    function _login_helper($username, $password = null)
    {
        ...
        switch ($type) {
            ...
            case NET_SSH2_MSG_USERAUTH_FAILURE:
                // can we use keyboard-interactive authentication?  if not then either the sessions is bad or the server employees
    ...
    function _privatekey_login($username, $privatekey)
    {
        ...
        switch ($type) {
            case NET_SSH2_MSG_USERAUTH_FAILURE:
                // either the sessions is bad or the server employs multi-factor authentication
                return false;
            case NET_SSH2_MSG_USERAUTH_SUCCESS:
                $this->bitmap |= self::MASK_LOGIN;
                return true;
    ...
    function read($expect = '', $mode = self::READ_SIMPLE)
    {
        ...
        if (!$this->isAuthenticated()) {
            user_error('Operation disallowed prior to sessions()');
            return false;
        }
